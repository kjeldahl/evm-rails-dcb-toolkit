...
1. Immediately call `/update-slice-status` to set the slice to `InProgress` on the board.
   ...
   **Claim conflict**: if this call reports the slice is already in `InProgress` (or any status other than `Planned`), another agent already claimed it first — this is expected, not an error. Log it in `progress.txt`, drop this task without building, and continue the loop. Do not retry.

2. Read the slice definition from `.build-kit/.slices/<contextSlug>/<sliceFolder>/slice.json` (written by `/load-slice`).

3. Determine the **slice type** — see `.build-kit/CLAUDE.md`'s own "Building a slice" section for the exact field-presence rules this stack uses (don't hard-fail on an unrecognized `sliceType`; fall through to field presence):
   - **an inbound external event** — an `events[]` element with `context: "EXTERNAL"` → `/build-webhook`
   - **Automation** — `processors` array is non-empty → invoke `/build-automation`
   - **State-view** — `readmodels` array is non-empty → invoke `/build-state-view`
